import player from './playerAniversario.js'
...
const eueoc = document.querySelector('div.MIAU')
let count_click = 0
...
document.getElementById('btn').addEventListener('click', () => {
    ...
    if(count_click <= 1) {
        ...
        setTimeout(typewriterParagraph, 3800)
        setTimeout(typewriterSpan, 59000)
        animatedPlay()
...
let miau = document.querySelector('.nossoMomento')

eueoc.addEventListener('click', () => {
    document.querySelector("video.video").classList.remove('off')
    document.querySelector("video.video").classList.add('on')
    console.log(miau)
})
